...
**Markora** 是一款基于强大的 CodeMirror 6 框架构建的现代、直观的 Markdown 编辑器，致力于为纯文本提供真正的 **所见即所得（WYSIWYG）** 体验。告别对原始 ~~Markdown 语法~~ 的分心；借助 _Markora_，你的文档会在输入时即时转化为美观、可读的预览，同时底层 Markdown 始终可访问、可编辑。
...
- **🌗 主题：** 一等公民式地支持浅色与深色模式。
- **📦 模块化导出：** 只导入你需要的内容（markora/editor、markora/preview、markora/plugins）。

### 示例：
到目前为止，我们已经看到了标题、图片和列表。接下来探索一些你可以借助 Markora 使用的其他特性。
...
#### HTML：
**Markora** 同时支持行内 **HTML**、**HTML 块** 和 **注释**。<span style="border: 1px solid; padding: 0 0.5rem;">行内 HTML</span>
...
#### Peer Dependencies
Markora 需要以下 CodeMirror 包作为 peer dependencies。请确保它们已安装在你的项目中：
\`\`\`shell title="npm"
...
import { allPlugin } from "markora/plugins";
...
总而言之，**Markora** 代表了 Markdown 编辑领域的一次重大飞跃，有效消除了书写与预览之间的屏障。借助 CodeMirror 6 现代、高性能的能力，它提供了一种流畅、实时的 WYSIWYG 体验，能够直观地渲染从基础排版到代码块、KaTeX 数学公式等复杂元素的一切内容。这种 Markdown 原始能力与富文本编辑器即时视觉反馈的无缝融合，创造了一个专注、高效且强大的环境，使其成为写作者、开发者和学者的卓越工具。
