<template>
  <div class="editor-host">
    ...
    <div v-else-if="mode === 'view' || mode === 'live'" class="preview-with-toc" :style="previewTocStyle">
      ...
      <aside v-if="config.features.tableOfContents" class="vue3-preview-toc">
        <div class="vue3-preview-toc-resize" @mousedown="startPreviewTocResize" />
        <nav class="vue3-preview-toc-list" aria-label="Document table of contents">
          <button
            v-for="item in previewToc"
            :key="item.id"
            type="button"
            class="vue3-preview-toc-item"
            :class="{ 'vue3-preview-toc-item-active': item.active }"
          ...
          <div v-if="previewToc.length === 0" class="vue3-preview-toc-empty">暂无目录</div>
        </nav>
      </aside>
    </div>
...
import { bindCodeCopyButtons, bindLinkPreviewCardButtons } from "mardora/plugins";
...
const previewTocStorageKey = "mardora-vue3-playground:preview-toc-width";
...
export default defineComponent({
  ...
  computed: {
    previewTocStyle(): Record<string, string> {
      return {
        "--vue3-preview-toc-width": `${this.previewTocWidth}px`,
        "--vue3-content-max-width":
  ...
  methods: {
    ...
    startPreviewTocResize(event: MouseEvent) {
      ...
      document.body.classList.add("vue3-preview-toc-resizing");

      const move = (moveEvent: MouseEvent) => {
        this.previewTocWidth = clampPreviewTocWidth(startWidth - (moveEvent.clientX - startX));
      };
      const up = () => {
        document.body.classList.remove("vue3-preview-toc-resizing");
