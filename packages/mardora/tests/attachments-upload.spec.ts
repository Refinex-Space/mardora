...
import type { MardoraAttachmentEditorView } from "../src/editor/attachments";
...
class FakeEditorView implements MardoraAttachmentEditorView {
  text: string;
  state: MardoraAttachmentEditorView["state"];
  ...
  dispatch(spec: Parameters<MardoraAttachmentEditorView["dispatch"]>[0]): void {
  ...
  private createState(text: string, anchor: number): MardoraAttachmentEditorView["state"] {
    return {
...
describe("uploadAttachmentFile", () => {
  ...
  it("keeps a failed marker on uploader rejection", async () => {
    ...
    expect(view.text).toContain("[Upload failed: a.pdf](mardora-upload://");
  });
