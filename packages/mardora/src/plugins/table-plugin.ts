...
import { MardoraConfig } from "../editor/mardora";
...
const tableBlockWrapper = BlockWrapper.create({
  tagName: "div",
  attributes: { class: "cm-mardora-table-wrapper" },
});
...
class TableBreakWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    span.className = "cm-mardora-table-break";
...
class TableCaretWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    const span = document.createElement("span");
    span.className = "cm-mardora-table-caret";
...
class TableControlsWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    anchor.className = "cm-mardora-table-controls-anchor";
    anchor.setAttribute("aria-hidden", "true");

    const rightButton = this.createButton("Add column", "cm-mardora-table-control cm-mardora-table-control-column");
    ...
    const bottomButton = this.createButton("Add row", "cm-mardora-table-control cm-mardora-table-control-row");
...
function createPreviewRenderer(markdown: string, config?: MardoraConfig): PreviewRenderer {
...
async function renderCellToHtml(text: string, config?: MardoraConfig): Promise<string> {
  if (!text.trim()) {
    return "&nbsp;";
  }
...
async function renderTableToHtml(parsed: ParsedTable, config?: MardoraConfig): Promise<string> {
  const normalized = normalizeParsedTable(parsed);
  let html = '<div class="cm-mardora-table-widget"><table class="cm-mardora-table cm-mardora-table-preview">';
  html += '<thead><tr class="cm-mardora-table-row cm-mardora-table-header-row">';
  ...
  for (let index = 0; index < normalized.headers.length; index++) {
    ...
    html += `<th class="cm-mardora-table-cell cm-mardora-table-th${
      alignment === "center"
        ? " cm-mardora-table-cell-center"
        : alignment === "right"
          ? " cm-mardora-table-cell-right"
          : ""
    }${index === normalized.headers.length - 1 ? " cm-mardora-table-cell-last" : ""}">${content}</th>`;
  }
  ...
  for (let rowIndex = 0; rowIndex < normalized.rows.length; rowIndex++) {
    ...
    html += `<tr class="cm-mardora-table-row cm-mardora-table-body-row${
      rowIndex === normalized.rows.length - 1 ? " cm-mardora-table-row-last" : ""
    }">`;

    for (let index = 0; index < normalized.headers.length; index++) {
      const alignment = normalized.alignments[index] || "left";
      const content = await renderCellToHtml(row[index] || "", config);
      html += `<td class="cm-mardora-table-cell${
        alignment === "center"
          ? " cm-mardora-table-cell-center"
          : alignment === "right"
            ? " cm-mardora-table-cell-right"
            : ""
      }${index === normalized.headers.length - 1 ? " cm-mardora-table-cell-last" : ""}">${content}</td>`;
    }
...
function getTableCellPositionFromPoint(view: EditorView, cellElement: Element, x: number, y: number): number | null {
  const contentFrom = Number(cellElement.getAttribute("data-mardora-content-from"));
  const contentTo = Number(cellElement.getAttribute("data-mardora-content-to"));
...
function getTablePositionFromPoint(view: EditorView, x: number, y: number): number | null {
  ...
  const cellElement = element?.closest(".cm-mardora-table-cell");
...
const lineDecorations = {
  header: Decoration.line({ class: "cm-mardora-table-row cm-mardora-table-header-row" }),
  delimiter: Decoration.line({ class: "cm-mardora-table-row cm-mardora-table-delimiter-row" }),
  body: Decoration.line({ class: "cm-mardora-table-row cm-mardora-table-body-row" }),
  last: Decoration.line({ class: "cm-mardora-table-row-last" }),
};
...
function getCellClassName(
  ...
  return [
    ...
    isHeader ? "cm-mardora-table-th" : "",
    alignment === "center" ? "cm-mardora-table-cell-center" : "",
    alignment === "right" ? "cm-mardora-table-cell-right" : "",
    isLastCell ? "cm-mardora-table-cell-last" : "",
...
function getCellDecoration(
  ...
  return Decoration.mark({
    ...
    attributes: {
      ...
      "data-mardora-table-from": String(tableFrom),
      ...
      "data-mardora-content-from": String(cell.contentFrom),
      "data-mardora-content-to": String(cell.contentTo),
    },
  });
}
...
export class TablePlugin extends DecorationPlugin {
  ...
  private mardoraConfig: MardoraConfig | undefined;
  ...
  override onRegister(context: PluginContext): void {
    ...
    this.mardoraConfig = context.config;
  }
  ...
  private handleTableMouseDown(view: EditorView, event: MouseEvent): boolean {
    ...
    const target = event.target instanceof Element ? event.target.closest(".cm-mardora-table-cell") : null;
  ...
  override async renderToHTML(node: SyntaxNode, _children: string, ctx: PreviewContextLike): Promise<string | null> {
    if (node.name === "Table") {
      ...
      const tableHtml = await renderTableToHtml(parsed, this.mardoraConfig);
      if (!trailingMarkdown.trim()) {
        return tableHtml;
      }

      return tableHtml + (await createPreviewRenderer(trailingMarkdown, this.mardoraConfig).render());
    }
...
const theme = createTheme({
  default: {
    ".cm-mardora-table-wrapper, .cm-mardora-table-widget": {
      display: "table",
      width: "100%",
      ...
      "& .cm-mardora-table-row": {
        display: "table-row !important",
      },

      "& .cm-mardora-table-delimiter-row": {
      ...
      "& .cm-mardora-table-body-row": {
        minHeight: "2.75rem",
      },

      "& .cm-mardora-table-cell .cm-mardora-code-inline": {
        whiteSpace: "normal",
      ...
      "& .cm-mardora-table-cell-last": {
        borderRight: "none",
      },

      "& .cm-mardora-table-row-last .cm-mardora-table-cell": {
        borderBottom: "none",
      },
      ...
      "& .cm-mardora-table-break": {
        display: "inline",
      },

      "& .cm-mardora-table-caret": {
      ...
      "& .cm-mardora-table-controls-anchor": {
        position: "absolute",
        inset: "0",
        pointerEvents: "none",
      },

      "& .cm-mardora-table-control": {
        position: "absolute",
        width: "1.25rem",
        height: "1.25rem",
        border: "1px solid var(--color-border, #d7dee7)",
      ...
      "& .cm-mardora-table-control:hover, & .cm-mardora-table-control:focus-visible": {
      ...
      "& .cm-mardora-table-control-column": {
        top: "50%",
        right: "-0.625rem",
        transform: "translate(0.25rem, -50%)",
      },

      "& .cm-mardora-table-control-row": {
        left: "50%",
        bottom: "-0.625rem",
        transform: "translate(-50%, 0.25rem)",
      },

      "&:hover .cm-mardora-table-control, &:focus-within .cm-mardora-table-control": {
        opacity: "1",
      },

      "&:hover .cm-mardora-table-control-column, &:focus-within .cm-mardora-table-control-column": {
        transform: "translate(0, -50%)",
      },

      "&:hover .cm-mardora-table-control-row, &:focus-within .cm-mardora-table-control-row": {
