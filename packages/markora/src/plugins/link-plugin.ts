...
const linkMarkDecorations = {
  "link-text": Decoration.mark({ class: "cm-markora-link-text" }),
  "link-marker": Decoration.mark({ class: "cm-markora-link-marker" }),
  "link-url": Decoration.mark({ class: "cm-markora-link-url" }),
  "link-hidden": Decoration.mark({ class: "cm-markora-link-hidden" }),
};
...
class LinkTooltipWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    wrapper.className = "cm-markora-link-wrapper";
    wrapper.style.cursor = "pointer";

    // Tooltip element
    const tooltip = document.createElement("span");
    tooltip.className = "cm-markora-link-tooltip";
    tooltip.textContent = this.url;
    wrapper.appendChild(tooltip);

    // Show/hide tooltip on hover
    wrapper.addEventListener("mouseenter", () => {
      tooltip.classList.add("cm-markora-link-tooltip-visible");
    });

    wrapper.addEventListener("mouseleave", () => {
      tooltip.classList.remove("cm-markora-link-tooltip-visible");
    });
...
export class LinkPlugin extends DecorationPlugin {
  ...
  override renderToHTML(
    ...
    return `<a class="cm-markora-link" href="${urlAttr}"${titleAttr} target="_blank" rel="noopener noreferrer">${textContent}</a>`;
  }
}
...
class LinkTextWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    span.className = "cm-markora-link-styled";
...
const theme = createTheme({
  default: {
    // Link text
    ".cm-markora-link-text": {
      color: "#0366d6",
    },

    // Link markers ([ ] ( ))
    ".cm-markora-link-marker": {
      color: "#6a737d",
    ...
    ".cm-markora-link-hidden": {
      display: "none",
    },

    // Styled link when cursor is not in range
    ".cm-markora-link-styled": {
      color: "#0366d6",
    ...
    ".cm-markora-link:hover": {
      color: "#0056b3",
    },

    // Tooltip styling
    ".cm-markora-link-tooltip": {
      display: "none",
      position: "absolute",
      bottom: "100%",
      left: "50%",
    ...
    ".cm-markora-link-tooltip-visible": {
      display: "block",
    },
  },
