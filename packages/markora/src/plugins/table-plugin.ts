...
const tableBlockWrapper = BlockWrapper.create({
  tagName: "div",
  attributes: { class: "cm-markora-table-wrapper" },
});
...
class TableBreakWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    span.className = "cm-markora-table-break";
...
class TableControlsWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    anchor.className = "cm-markora-table-controls-anchor";
    anchor.setAttribute("aria-hidden", "true");

    const rightButton = this.createButton("Add column", "cm-markora-table-control cm-markora-table-control-column");
    ...
    const bottomButton = this.createButton("Add row", "cm-markora-table-control cm-markora-table-control-row");
...
function createPreviewRenderer(markdown: string, config?: MarkoraConfig): PreviewRenderer {
...
async function renderCellToHtml(text: string, config?: MarkoraConfig): Promise<string> {
  if (!text.trim()) {
    return "&nbsp;";
  }
...
async function renderTableToHtml(parsed: ParsedTable, config?: MarkoraConfig): Promise<string> {
  const normalized = normalizeParsedTable(parsed);
  let html = '<div class="cm-markora-table-widget"><table class="cm-markora-table cm-markora-table-preview">';
  html += '<thead><tr class="cm-markora-table-row cm-markora-table-header-row">';
  ...
  for (let index = 0; index < normalized.headers.length; index++) {
    ...
    html += `<th class="cm-markora-table-cell cm-markora-table-th${
      alignment === "center"
        ? " cm-markora-table-cell-center"
        : alignment === "right"
          ? " cm-markora-table-cell-right"
          : ""
    }${index === normalized.headers.length - 1 ? " cm-markora-table-cell-last" : ""}">${content}</th>`;
  }
  ...
  for (let rowIndex = 0; rowIndex < normalized.rows.length; rowIndex++) {
    ...
    for (let index = 0; index < normalized.headers.length; index++) {
      ...
      html += `<td class="cm-markora-table-cell${
        alignment === "center"
          ? " cm-markora-table-cell-center"
          : alignment === "right"
            ? " cm-markora-table-cell-right"
            : ""
      }${index === normalized.headers.length - 1 ? " cm-markora-table-cell-last" : ""}">${content}</td>`;
    }
...
export class TablePlugin extends DecorationPlugin {
  ...
  private markoraConfig: MarkoraConfig | undefined;
  ...
  override onRegister(context: PluginContext): void {
    ...
    this.markoraConfig = context.config;
  }
  ...
  override async renderToHTML(node: SyntaxNode, _children: string, ctx: PreviewContextLike): Promise<string | null> {
    if (node.name === "Table") {
      ...
      const tableHtml = await renderTableToHtml(parsed, this.markoraConfig);
      if (!trailingMarkdown.trim()) {
        return tableHtml;
      }

      return tableHtml + (await createPreviewRenderer(trailingMarkdown, this.markoraConfig).render());
    }
...
const theme = createTheme({
  default: {
    ".cm-markora-table-wrapper, .cm-markora-table-widget": {
      ...
      "& .cm-markora-table-body-row": {
        minHeight: "2.75rem",
      },

      "& .cm-markora-table-cell .cm-markora-code-inline": {
        whiteSpace: "normal",
      ...
      "& .cm-markora-table-row-last .cm-markora-table-cell": {
        borderBottom: "none",
      },
      ...
      "&:hover .cm-markora-table-control-column, &:focus-within .cm-markora-table-control-column": {
        transform: "translate(0, -50%)",
      },

      "&:hover .cm-markora-table-control-row, &:focus-within .cm-markora-table-control-row": {
