...
class EmojiWidget extends WidgetType {
  ...
  toDOM() {
    ...
    span.className = "cm-markora-emoji";
...
const emojiMarkDecorations = {
  "emoji-source": Decoration.mark({ class: "cm-markora-emoji-source" }),
};
...
export class EmojiPlugin extends DecorationPlugin {
  ...
  override renderToHTML(
    ...
    const rendered = shortcodeToEmoji(raw);
    if (!rendered) {
      return `<span class="cm-markora-emoji-source">${children}</span>`;
    }

    return `<span class="cm-markora-emoji">${rendered}</span>`;
  }
}
