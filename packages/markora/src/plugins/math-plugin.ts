...
function injectKatexStyles(): void {
  ...
  if (document.getElementById("markora-katex-styles")) return;

  const style = document.createElement("style");
  style.id = "markora-katex-styles";
...
const mathMarkDecorations = {
  "math-block": Decoration.line({ class: "cm-markora-math-block" }),
  "math-inline": Decoration.mark({ class: "cm-markora-math-inline" }),
  "math-marker": Decoration.mark({ class: "cm-markora-math-marker" }),
  "math-hidden": Decoration.mark({ class: "cm-markora-math-hidden" }),
};
...
class InlineMathWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    span.className = "cm-markora-math-rendered cm-markora-math-rendered-inline";
    span.style.cursor = "pointer";

    const { html, error } = renderMath(this.latex, false);

    if (error) {
      span.className += " cm-markora-math-error";
...
class MathBlockWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    div.className = "cm-markora-math-rendered cm-markora-math-rendered-block";
    div.style.cursor = "pointer";

    const { html, error } = renderMath(this.latex, true);

    if (error) {
      div.className += " cm-markora-math-error";
...
export class MathPlugin extends DecorationPlugin {
  ...
  override renderToHTML(
    ...
    if (node.name === "InlineMath") {
      ...
      if (error) {
        return `<span class="cm-markora-math-error">[Math Error: ${ctx.sanitize(error)}]</span>`;
      }
      return `<span class="cm-markora-math-rendered cm-markora-math-rendered-inline">${html}</span>`;
    }
    ...
    if (node.name === "MathBlock") {
      ...
      if (error) {
        return `<div class="cm-markora-math-error">[Math Error: ${ctx.sanitize(error)}]</div>`;
      }
      return `<div class="cm-markora-math-rendered cm-markora-math-rendered-block">${html}</div>`;
    }
