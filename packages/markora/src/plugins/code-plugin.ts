...
const codeMarkDecorations = {
  // Inline code
  "inline-code": Decoration.mark({ class: "cm-markora-code-inline" }),
  ...
  // Highlights
  "code-line-highlight": Decoration.line({ class: "cm-markora-code-line-highlight" }),
  "code-text-highlight": Decoration.mark({ class: "cm-markora-code-text-highlight" }),

  // Diff preview
  "diff-line-add": Decoration.line({ class: "cm-markora-code-line-diff-add" }),
  "diff-line-del": Decoration.line({ class: "cm-markora-code-line-diff-del" }),
  "diff-sign-add": Decoration.mark({ class: "cm-markora-code-diff-sign-add" }),
  "diff-sign-del": Decoration.mark({ class: "cm-markora-code-diff-sign-del" }),
  "diff-mod-add": Decoration.mark({ class: "cm-markora-code-diff-mod-add" }),
  "diff-mod-del": Decoration.mark({ class: "cm-markora-code-diff-mod-del" }),
...
class CodeBlockCaptionWidget extends WidgetType {
  ...
  toDOM(): HTMLElement {
    ...
    captionEl.className = "cm-markora-code-caption";
...
export class CodePlugin extends DecorationPlugin {
  ...
  private decorateFencedCode(node: { from: number; to: number; node: SyntaxNode }, ctx: DecorationContext): void {
    ...
    for (let lineNumber = nodeLineStart.number; lineNumber <= nodeLineEnd.number; lineNumber++) {
      ...
      if (!isFenceLine && infoProps.showLineNumbers && !infoProps.diff) {
        decorations.push(
          Decoration.line({
            class: "cm-markora-code-line-numbered",
            attributes: {
      ...
      if (!isFenceLine && infoProps.showLineNumbers && infoProps.diff) {
        ...
        decorations.push(
          Decoration.line({
            class: "cm-markora-code-line-numbered-diff",
            attributes: {
  ...
  private decorateDiffLine(
    ...
    if (showDiffMarkerGutter) {
      decorations.push(
        Decoration.line({
          class: "cm-markora-code-line-diff-gutter",
          attributes: {
            "data-diff-marker": diffMarker,
          },
        }).range(line.from)
  ...
  override async renderToHTML(node: SyntaxNode, _children: string, ctx: PreviewRenderContext): Promise<string | null> {
    ...
    if (node.name === "InlineCode") {
      ...
      return `<code class="cm-markora-code-inline" style="padding: 0.1rem 0.25rem">${this.escapeHtml(content)}</code>`;
    }
    ...
    if (node.name === "FencedCode") {
      ...
      codeLines.forEach((line, index) => {
        ...
        // Line classes
        const lineClasses: string[] = ["cm-markora-code-line"];
        if (isHighlighted) lineClasses.push("cm-markora-code-line-highlight");
        if (props.showLineNumbers) {
          lineClasses.push(props.diff ? "cm-markora-code-line-numbered-diff" : "cm-markora-code-line-numbered");
        }
        if (diffState?.kind === "addition") lineClasses.push("cm-markora-code-line-diff-add");
        if (diffState?.kind === "deletion") lineClasses.push("cm-markora-code-line-diff-del");
        ...
        if (props.diff) {
          ...
          if (props.showLineNumbers) {
            ...
            lineAttrs.push(`data-diff-marker="${diffMarker}"`);
            lineClasses.push("cm-markora-code-line-diff-gutter");
      ...
      // Caption
      if (props.caption) {
        html += `<div class="cm-markora-code-caption">${this.escapeHtml(props.caption)}</div>`;
      }
  ...
  private renderDiffPreviewLine(diffState: DiffLineState, highlightedContent: string): string {
    const modClass =
      diffState.kind === "addition"
        ? "cm-markora-code-diff-mod-add"
        : diffState.kind === "deletion"
          ? "cm-markora-code-diff-mod-del"
          : "";
    ...
    for (const [highlightIndex, highlight] of highlights.entries()) {
      try {
        ...
        result = result.replace(regex, (match) => {
          ...
          if (shouldHighlight) {
            return `<mark class="cm-markora-code-text-highlight">${match}</mark>`;
          }
          return match;
        });
