...
async function renderMermaid(
  ...
  try {
    const id = `markora-mermaid-${mermaidCounter++}`;
...
const mermaidMarkDecorations = {
  "mermaid-block-start": Decoration.line({ class: "cm-markora-mermaid-block-start" }),
  "mermaid-block-end": Decoration.line({ class: "cm-markora-mermaid-block-end" }),
  "mermaid-block": Decoration.line({ class: "cm-markora-mermaid-block" }),
  "mermaid-block-rendered": Decoration.line({ class: "cm-markora-mermaid-block-rendered" }),
  "mermaid-marker": Decoration.mark({ class: "cm-markora-mermaid-marker" }),
  "mermaid-hidden": Decoration.mark({ class: "cm-markora-mermaid-hidden" }),
};
...
class MermaidBlockWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    div.innerHTML = `<div class="cm-markora-mermaid-loading">Rendering diagram…</div>`;

    // Render mermaid asynchronously
    // Render mermaid asynchronously
    renderMermaid(this.definition, this.attributes, this.defaultTheme).then(({ svg, error }) => {
      if (error) {
        div.className += " cm-markora-mermaid-error";
...
export class MermaidPlugin extends DecorationPlugin {
  ...
  override async renderToHTML(
    ...
    if (node.name === "MermaidBlock") {
      ...
      if (error) {
        return `<div class="cm-markora-mermaid-error">${ctx.sanitize(`[Mermaid Error: ${error}]`)}</div>`;
      }

      return `<div class="cm-markora-mermaid-rendered">${svg}</div>`;
    }
...
const theme = createTheme({
  default: {
    ...
    ".cm-markora-mermaid-block-start:not(.cm-markora-mermaid-block-rendered)": {
      overflow: "hidden",
    ...
    ".cm-markora-mermaid-block-end:not(.cm-markora-mermaid-block-rendered)": {
      overflow: "hidden",
    ...
    ".cm-markora-mermaid-block:not(.cm-markora-mermaid-block-rendered)::before": {
    ...
    ".cm-markora-mermaid-block.cm-markora-mermaid-block-rendered br": {
      display: "none",
    },

    // Mermaid markers (```mermaid / ```)
    ".cm-markora-mermaid-marker": {
      color: "#6a737d",
      fontFamily: "var(--font-jetbrains-mono, monospace)",
    },

    // Hidden mermaid syntax (when cursor is not in range)
    ".cm-markora-mermaid-hidden": {
      display: "none",
    },

    // Rendered mermaid container
    ".cm-markora-mermaid-rendered": {
      display: "flex",
    ...
    ".cm-markora-mermaid-rendered svg": {
      maxWidth: "100%",
      height: "auto",
      aspectRatio: "auto",
    },
