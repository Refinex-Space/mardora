...
const classes = {
  ...
  lineUL: "cm-markora-list-line-ul",
  markUL: "cm-markora-list-mark-ul",

  // Ordered list classes
  lineOL: "cm-markora-list-line-ol",
  markOL: "cm-markora-list-mark-ol",

  // Task list classes
  taskLine: "cm-markora-task-line",
  taskMarker: "cm-markora-task-marker",

  // Common classes
  content: "cm-markora-list-content",
  indent: "cm-markora-list-indent",
  active: " cm-markora-active",
  preview: "cm-markora-preview",
};
...
export class TaskCheckboxWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    wrap.className = `cm-markora-task-checkbox ${this.checked ? "checked" : ""}`;
  ...
  override renderToHTML(
    ...
    switch (node.name) {
      ...
      case "TaskMarker": {
        ...
        const isChecked = text.includes("x") || text.includes("X");
        return `<input type="checkbox" class="cm-markora-task-checkbox" disabled ${isChecked ? "checked" : ""} />`;
      }
...
const theme = createTheme({
  default: {
    ...
    ".cm-markora-list-line-ul, .cm-markora-list-line-ol": {
      position: "relative",
      paddingLeft: "calc(1rem * (var(--depth, 0) + 1)) !important",
      display: "flex",
      alignItems: "start",
    },
    ".cm-markora-list-line-ul > :first-child, .cm-markora-list-line-ol > :first-child": {
      flexShrink: 0,
    },

    // List marker sizing
    ".cm-markora-list-line-ul .cm-markora-list-mark-ul, .cm-markora-list-line-ol .cm-markora-list-mark-ol": {
      whiteSpace: "pre",
      position: "relative",
      width: "1rem",
      flexShrink: 0,
    ...
    ".cm-markora-list-mark-ul:not(.cm-markora-active) > span, .cm-markora-task-line .cm-markora-list-mark-ol:not(.cm-markora-active) > span":
      {
        visibility: "hidden",
        display: "none",
      },
    ...
    ".cm-markora-list-line-ul .cm-markora-list-mark-ul:not(.cm-markora-active)::after": {
      content: '"•"',
    ...
    ".cm-markora-task-marker": {
      color: "var(--markora-highlight, #a4a4a4)",
      fontFamily: "monospace",
    },

    // Task checkbox container
    ".cm-markora-task-checkbox": {
      display: "inline-flex",
    ...
    ".cm-markora-task-checkbox input": {
      cursor: "pointer",
      margin: 0,
      width: "1.1em",
      height: "1.1em",
    ...
    ".cm-markora-task-checkbox.checked input::after": {
      content: '"✓"',
      position: "absolute",
      left: "1px",
      top: "-3px",
    ...
    ".cm-markora-preview li": {
      display: "list-item",
      marginBottom: "0.25rem",
    },
    "ul.cm-markora-preview": {
      listStyleType: "disc",
    },
    "ol.cm-markora-preview": {
      listStyleType: "decimal",
    },
    // Hide list marker for task items
    ".cm-markora-preview li:has(.cm-markora-task-checkbox)": {
      listStyleType: "none",
    },
    ".cm-markora-preview li .cm-markora-paragraph": {
      padding: "0",
    },
  },
});
