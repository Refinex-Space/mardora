...
import { MarkoraPlugin, PluginContext } from "./plugin";
import { createMarkoraViewExtension } from "./view-plugin";
...
/**
 * MarkoraNode: represents a node in the markdown tree
 *
...
/**
 * Configuration options for the markora editor
 */
export interface MarkoraConfig {
  /** Theme */
  theme?: ThemeEnum;
...
/**
 * Creates a markora editor extension bundle for CodeMirror 6
 *
 ...
 * ```
 */
export function markora(config: MarkoraConfig = {}): Extension[] {
  const {
    locale: configLocale,
    i18n: configI18n = {},
  ...
  // markora extensions (pass plugins for decoration support)
  const markoraExtensions: Extension[] = [];
  if (!disableViewPlugin) {
    markoraExtensions.push(createMarkoraViewExtension(configTheme, baseStyles, allPlugins, configOnNodesChange));
    markoraExtensions.push(Prec.highest(markdownResetExtension));
  }
  if (!disableViewPlugin || configLineWrapping) markoraExtensions.push(EditorView.lineWrapping);
  ...
  const composedExtensions: Extension[] = [
    ...
    // markora view plugin for rich rendering
    markoraExtensions,

    // Core CodeMirror extensions
    baseExtensions,

    // Markora editor commands and browser attachments
    slashCommands({
      ...configSlashCommands,
