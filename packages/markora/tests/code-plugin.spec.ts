...
describe("code plugin", () => {
  ...
  it("renders preview code blocks with the compact hover toolbar instead of a solid header", async () => {
    ...
    expect(html).toContain("cm-markora-code-container");
    expect(html).toContain("cm-markora-code-toolbar");
    expect(html).toContain("cm-markora-code-language-button");
    expect(html).toContain("cm-markora-code-copy-btn");
    expect(html).toContain("data-lang=\"ts\"");
    expect(html).not.toContain("cm-markora-code-header");
  });
  ...
  it("generates transparent bordered code block styles with hover tools", () => {
    ...
    expect(css).toContain(".cm-markora-code-toolbar");
    expect(css).toContain("opacity: 0;");
    expect(css).toContain("background-color: transparent;");
    expect(css).toContain("border: 1px solid var(--color-border, #d4d4d8);");
    expect(css).toContain(".cm-markora-code-fence-line");
  ...
  it("keeps single-line code blocks vertically balanced with floating tools", () => {
    ...
    expect(css).toContain(".cm-markora-code-block-single-line");
    expect(css).toContain(".cm-markora-code-block-rendered.cm-markora-code-block-single-line");
  ...
  it("keeps empty fenced code blocks tall enough for direct input", () => {
    ...
    expect(css).toContain(".cm-markora-code-block-line {");
    expect(css).toContain("min-height: 1.5em;");
    expect(css).not.toContain(".cm-markora-code-block-line-end br {display: none;}");
  });
