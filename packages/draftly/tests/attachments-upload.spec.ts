...
import type { DraftlyAttachmentEditorView } from "../src/editor/attachments";
...
class FakeEditorView implements DraftlyAttachmentEditorView {
  text: string;
  state: DraftlyAttachmentEditorView["state"];
  ...
  dispatch(spec: Parameters<DraftlyAttachmentEditorView["dispatch"]>[0]): void {
  ...
  private createState(text: string, anchor: number): DraftlyAttachmentEditorView["state"] {
    return {
...
describe("uploadAttachmentFile", () => {
  ...
  it("keeps a failed marker on uploader rejection", async () => {
    ...
    expect(view.text).toContain("[Upload failed: a.pdf](draftly-upload://");
  });
