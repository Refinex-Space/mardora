type IconElementName = "circle" | "path" | "rect";
...
const iconDefinitions: Record<DraftlyIconName, IconElementDefinition[]> = {
  file: [
    {
      name: "path",
      attrs: {
...
export function hasDraftlyIcon(name: string): name is DraftlyIconName {
  return name in iconDefinitions;
}

export function createDraftlyIcon(name: string): SVGSVGElement | null {
  if (!hasDraftlyIcon(name)) return null;
