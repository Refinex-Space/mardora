...
为 Draftly 设计内置目录能力。该能力默认开启，接入方可以直接使用 Draftly 提供的右侧目录 UI；如果接入方希望自定义目录，也可以关闭默认 UI，通过 `onTocChange` 获取结构化目录数据自行渲染。

目录应覆盖 Live 编辑模式和 View 预览模式，渲染范围为 `h2` 到 `h6`，不包含 `h1`。

## Context

当前 Draftly 的交互型编辑器能力位于 `packages/draftly/src/editor/*`，例如 slash menu、附件上传、选中文本工具条；`packages/draftly/src/plugins/*` 主要用于 Markdown 节点装饰与预览渲染。因此目录能力应作为内置 editor/preview 能力设计，而不是放入现有 Markdown plugin 目录。

Vue2 playground 的 `Contents`、`Developer Panel`、顶部工具栏只是演示站点外壳，不属于 `packages/draftly` 对接入方交付的能力。接入方实际感知的是中间 Draftly 编辑/预览区域，目录应出现在该区域右侧。
...
- 目录覆盖 Live 和 View 模式。
- 目录状态默认不持久化；只有配置 `storageKey` 时才由 Draftly 写入 localStorage。
- 持久化只保存展开状态和宽度，不保存目录数据。

## Public API

在 `DraftlyConfig` 中新增 `toc` 配置：
...
- `toc.storageKey` 不传时，Draftly 不写宿主 localStorage。
...
- `packages/draftly/src/editor/table-of-contents/types.ts`
  定义公开配置、目录项、内部状态类型。

- `packages/draftly/src/editor/table-of-contents/slug.ts`
  负责 heading slug 与重复 id 去重。

- `packages/draftly/src/editor/table-of-contents/extract.ts`
  从 Live 模式的 CodeMirror state / Lezer tree 提取 `DraftlyTocItem`。

- `packages/draftly/src/editor/table-of-contents/extension.ts`
  Live 模式 ViewPlugin，负责监听文档、滚动、视口变化，计算目录项、active 项，渲染默认目录面板。

- `packages/draftly/src/editor/table-of-contents/panel.ts`
  原生 DOM 构建目录 Panel，不依赖 Vue 或 React。

- `packages/draftly/src/editor/table-of-contents/theme.ts`
  内置 Panel 样式。

- `packages/draftly/src/editor/table-of-contents/storage.ts`
  处理 `storageKey` 对应的展开状态和宽度读写。

- `packages/draftly/src/preview/toc.ts`
  View 模式提取 heading、补 id、生成目录数据。

- `packages/draftly/src/editor/draftly.ts`
  增加 `toc` 配置，默认启用内置目录扩展。

- `apps/vue2-playground`
