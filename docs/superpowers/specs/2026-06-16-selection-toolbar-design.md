...
在 Draftly 核心编辑器层实现类似 Notion、飞书、语雀的选中文本工具条。工具条在用户选中非空文本时出现，提供常用行内格式、链接、列表、文字颜色和高亮能力，并保持与 slash 菜单一致的黑白、紧凑、轻量浮层质感。

## Context

当前 Draftly 已具备：

- `packages/draftly/src/editor/slash`：slash 菜单的浮层、定位、键盘交互和主题。
- `packages/draftly/src/editor/icons`：内置 SVG 图标库。
- `packages/draftly/src/plugins/inline-plugin.ts`：加粗、斜体、删除线、高亮等 Markdown 行内渲染和快捷键。
- `packages/draftly/src/plugins/link-plugin.ts`：Markdown 链接渲染、编辑和 `Mod-k` 快捷键。
- `packages/draftly/src/plugins/list-plugin.ts`：有序列表、无序列表、任务列表转换和渲染。
- Vue2 playground 通过 `draftly()` 配置接入核心能力，不应实现私有工具条逻辑。

本设计将选中文本工具条作为 Draftly 核心能力交付，确保 React、Vue2、Vue3 等接入方能力一致。
...
- `packages/draftly/src/editor/selection-toolbar/extension.ts`
  - CodeMirror `ViewPlugin`。
  - 监听选区、文档、viewport 和焦点变化。
  - 控制工具条显示、关闭、重新定位和命令执行。

- `packages/draftly/src/editor/selection-toolbar/commands.ts`
  - 纯命令层。
  - 负责把选区转换为 Markdown/HTML。
  - 覆盖包裹、移除、链接解析、列表行转换等逻辑。

- `packages/draftly/src/editor/selection-toolbar/menu.ts`
  - DOM 渲染层。
  - 渲染工具条、按钮、分隔线、链接面板、颜色面板。
  - 使用内置 SVG 图标，不使用 emoji。

- `packages/draftly/src/editor/selection-toolbar/position.ts`
  - 浮层定位纯函数。
  - 计算工具条和子面板相对选区的最佳方向、左右贴边和最大尺寸。

- `packages/draftly/src/editor/selection-toolbar/theme.ts`
  - 工具条基础样式。
  - 与 slash 菜单保持一致的黑白视觉语言、圆角、阴影、按钮尺寸和暗色主题变量。

- `packages/draftly/src/editor/icons/index.ts`
...
新增 `packages/draftly/tests/selection-toolbar-commands.spec.ts`：
...
新增 `packages/draftly/tests/selection-toolbar-position.spec.ts`：
...
如需回滚，删除 `selection-toolbar` 模块，移除 `DraftlyConfig.selectionToolbar` 和 `draftly()` 中的扩展注册，并保留已有 slash 菜单、图标库和插件行为不变。
