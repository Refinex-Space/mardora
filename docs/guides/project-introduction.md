...
Markora 发布包名为 `@refinex/markora`。核心 CodeMirror 6 依赖由 Markora 作为 dependencies 固定并随包安装，业务项目只需安装 Markora。

```shell
npm install @refinex/markora
```

如果业务自己实现类似 playground 的 HTML/CSS 输出面板，可按需选择安装对应的 CodeMirror language/theme 包；Markora 核心编辑与预览不需要这些包。
...
| `headingFold`         | `MarkoraHeadingFoldConfig`       | `{ enabled: true }`  | Live 编辑态标题分节折叠。                   | 长文默认开启；源码模式或自定义折叠 UI 可关闭。         |
...
| `selectionToolbar.locale`  | `"zh-CN" \| "en-US"`  | 继承全局值  | 覆盖选区工具栏、链接面板和块类型菜单文案。 |
...
### 12.1 GFM Callout 告警块

`QuotePlugin` 兼容 GitHub Flavored Markdown 的 Callout 语法。Callout 本质上仍然保存为标准 Markdown 引用块，Markora 在编辑态和 preview 中识别第一行控制标记并渲染为告警块。
...
支持的类型包括 `NOTE`、`TIP`、`IMPORTANT`、`WARNING` 和 `CAUTION`，大小写不敏感。静态 preview 会隐藏 `[!TYPE]` 控制行，并输出 `cm-markora-callout` 与 `cm-markora-callout-{type}` 类名；普通 `>` 引用块保持原有渲染。
